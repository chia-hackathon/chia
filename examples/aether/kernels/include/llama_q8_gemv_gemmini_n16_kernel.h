...
//
// llama-q8-gemv-gemmini (N = 16) — THE FILE UNDER OPTIMIZATION.
//
...
//
// N = 16 fills exactly one row-tile of the array: the best case a batched decode server can reach before it becomes a GEMM.
//
// Baseline: the stock `tiled_matmul_auto` call, weight-stationary, no bias,
// int32 (`full_C`) output.

#ifndef LLAMA_Q8_GEMV_GEMMINI_N16_KERNEL_H
#define LLAMA_Q8_GEMV_GEMMINI_N16_KERNEL_H
...
//
// `dim_I = 16` is NOT a multiple of DIM = 16. `tiled_matmul_auto` rounds it up
...
#endif  // LLAMA_Q8_GEMV_GEMMINI_N16_KERNEL_H
