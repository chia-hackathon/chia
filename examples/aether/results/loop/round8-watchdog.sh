...
#     new run_id in round8-runs.txt.
BASE=/share1/saves/max410011/hackathon/aether
PY=/home/max410011_l/.conda/envs/chia_env/bin/python
OUT=$BASE/out/loop/round8-watchdog.log
RUNS=$BASE/out/loop/round8-runs.txt
cd "$BASE" || exit 1
...
# seed 策略跟 round8-launch.sh 必須一致：新 kernel 不加 --seed，
...
relaunch() {
  local kern=$1 iters=$2 log=$BASE/out/loop/round8-$kern.log
  local -a seedargs=()
...
while true; do
  ...
  for kern in "${KERNELS[@]}"; do
    log_f="$BASE/out/loop/round8-$kern.log"
  ...
  if (( ALIVE == 0 )); then
    log "all round8 runs done, watchdog stopping"
    break
  fi
  sleep 300
done
