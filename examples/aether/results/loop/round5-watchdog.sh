...
relaunch() {
  local kern=$1 iters=$2 log=$BASE/out/loop/round5-$kern.log
...
while true; do
  ...
  for kern in "${KERNELS[@]}"; do
    log_f="$BASE/out/loop/round5-$kern.log"
  ...
  if (( ALIVE == 0 )); then
    log "all round5 runs done, watchdog stopping"
    break
  fi
  sleep 300
done
