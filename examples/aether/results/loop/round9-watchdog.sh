...
#     new run_id in round9-runs.txt.
BASE=/share1/saves/max410011/hackathon/aether
PY=/home/max410011_l/.conda/envs/chia_env/bin/python
OUT=$BASE/out/loop/round9-watchdog.log
RUNS=$BASE/out/loop/round9-runs.txt
cd "$BASE" || exit 1
export RAY_ADDRESS=140.113.241.75:6379   # 本機有兩個 Ray cluster（titan 6380），明確指定

KERNELS=(
  llama-layer-fused-n1
)
declare -A PLANNED=(
  [llama-layer-fused-n1]=4
)
# seed 策略跟 round9-launch.sh 必須一致：新 kernel 不加 --seed，
# 重啟時若誤加 --seed best，會把前面幾個 iteration 的產物當成起點，
# baseline 分母不變但實驗語意就變了。
declare -A SEED=(
  [llama-layer-fused-n1]=best
  [llama-q8-gemv-gemmini-n1]=best
)
...
relaunch() {
  local kern=$1 iters=$2 log=$BASE/out/loop/round9-$kern.log
  local -a seedargs=()
...
while true; do
  ...
  for kern in "${KERNELS[@]}"; do
    log_f="$BASE/out/loop/round9-$kern.log"
  ...
  if (( ALIVE == 0 )); then
    log "all round9 runs done, watchdog stopping"
    break
  fi
  sleep 300
done
