...
{
echo "# round9 launched $(date -Is)  (--budget-usd 1000; llama-layer-fused-n1 是新 kernel，不加 --seed)"
} >> out/loop/round9-runs.txt
# round9 起，每個 kernel 的 seed 策略不同，所以 launch() 多吃一個參數：
#   ""      -> 不加 --seed（新 kernel，沒有可繼承的最佳版本）
#   "best"  -> --seed best（DB 裡該 kernel+config 的最佳通過版本）
launch() {
  kern=$1; iters=$2; seed=$3
  log="$BASE/out/loop/round9-$kern.log"
  seedargs=()
...
launch llama-layer-fused-n1 4 best   # Round 9: 從 179,585 起跳，專攻藏起剩下的 17,898 cycles Saturn 尾巴
sleep 60
