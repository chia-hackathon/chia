...
#     new run_id in round6-runs.txt.
BASE=/share1/saves/max410011/hackathon/aether
PY=/home/max410011_l/.conda/envs/chia_env/bin/python
OUT=$BASE/out/loop/round6-watchdog.log
RUNS=$BASE/out/loop/round6-runs.txt
cd "$BASE" || exit 1
...
relaunch() {
  local kern=$1 iters=$2 log=$BASE/out/loop/round6-$kern.log
...
while true; do
  ...
  for kern in "${KERNELS[@]}"; do
    log_f="$BASE/out/loop/round6-$kern.log"
  ...
  if (( ALIVE == 0 )); then
    log "all round6 runs done, watchdog stopping"
    break
  fi
  sleep 300
done
