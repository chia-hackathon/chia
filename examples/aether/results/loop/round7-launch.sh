...
{
echo "# round7 launched $(date -Is)  (--budget-usd 1000; llama-layer-fused-n1 是新 kernel，不加 --seed)"
} >> out/loop/round7-runs.txt
# round7 起，每個 kernel 的 seed 策略不同，所以 launch() 多吃一個參數：
#   ""      -> 不加 --seed（新 kernel，沒有可繼承的最佳版本）
#   "best"  -> --seed best（DB 裡該 kernel+config 的最佳通過版本）
launch() {
  kern=$1; iters=$2; seed=$3
  log="$BASE/out/loop/round7-$kern.log"
  seedargs=()
...
launch llama-layer-fused-n1 8 ""
sleep 60
# 探針 iteration：in-flight 6/8/10/12/16 掃描 + 冷態量測。數字比 best 重要。
launch llama-q8-gemv-gemmini-n1 2 best
