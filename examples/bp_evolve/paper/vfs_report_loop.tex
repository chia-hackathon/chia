% ============================================================================
% VARIANT of vfs_report_4p.tex, generated 2026-09-25.
% Differences from vfs_report_4p.tex:
%   * Sec. IV "What Moves the Ceiling" is deleted in full, with Table III
%     (tab:arms), the bounds/llm-par. arms, the 226-evaluation count, the
%     768-point box probe and the P2 separation.  Fig. 2's shaded P2 strip and
%     its caption clause went with it; the abstract's closing operator
%     paragraph and the intro's fourth contribution are gone (four
%     contributions -> three), and the Discussion's limitations paragraph no
%     longer names arms this version does not report.
%   * Fig. 1 (fig:loop) is new: a TikZ diagram of one generation of the loop,
%     placed in Sec. II.
%   * No gem5 anywhere (Tier 2 was removed from the loop long before this).
% Everything else -- Tables I and II, the elasticity analysis, the depth audit
% and the selection replay -- is byte-identical to vfs_report_4p.tex.
% ============================================================================
...
CHIA (CBP-NG and a ChampSim port check) to run MAP-Elites searches over the
reference TAGE under proposers of differing reach. Throughput
dominance turns out to be a property of the operating region rather than of the
metric: the throughput elasticity of \vfs{} lies between $+0.28$ and $+0.68$ for
every design our parameter search reached, yet falls to between $-0.02$ and $+0.06$ at the
...
are kept and which design is best.
\end{abstract}

\section{Introduction}

Agentic and evolutionary design loops~\cite{alphaevolve,archgym,funsearch,elm}
...
We audit both the score and the loop that optimizes it, and make three
...
falls to near zero or reverses at the winning entries. Second, we show that
whether a ranking of designs survives a change in pipeline depth is governed by
how widely the population spreads in accuracy, and that Eq.~\eqref{eq:cpi}
makes the test free.
...
best. Because two of the three archives the depth audit compares
were built by a rule-based proposer with no model in the loop, what it measures
is the objective rather than a model's priors. The loop, which connects two
toolchains through a shared lineage database on CHIA~\cite{chia}, is open source.

\section{The Design Loop}
\label{sec:loop}

The loop (Fig.~\ref{fig:loop}) is a graph of CHIA nodes, each bound to its own
container image and machine pool, through which candidates pass two evaluation
tiers of increasing cost. A proposer draws a parent uniformly from the archive's occupied cells and
...
\begin{tikzpicture}[font=\scriptsize,
  box/.style={draw,rounded corners=1.5pt,align=center,inner sep=2.5pt,
              minimum height=7.5mm,text width=16mm},
  db/.style={draw,dashed,rounded corners=1.5pt,align=center,inner sep=2pt,
             minimum height=4.5mm,text width=18mm,font=\scriptsize},
  ar/.style={-{Latex[length=1.5mm,width=1.2mm]},shorten >=0.5pt},
  lbl/.style={font=\scriptsize,inner sep=1.5pt}]
\node[box] (prop) at (0,2.0)    {proposer\\rule or agent};
\node[box] (bld)  at (3.1,2.0)  {lint,\\build};
\node[box] (t0)   at (6.2,2.0)  {Tier 0\\CBP-NG};
\node[box] (arch) at (0,0.5)    {archive\\$(\epi{},p_1,p_2)$ cells};
\node[box] (desc) at (6.2,0.5)  {\vfs{}\\+ descriptor};
\node[box] (t1)   at (3.1,-1.1) {Tier 1\\ChampSim port};
\node[db]  (db)   at (3.1,1.15) {lineage database};
\draw[ar] (arch) -- node[lbl,left]{parent} (prop);
\draw[ar] (prop) -- node[lbl,above]{source} (bld);
\draw[ar] (bld)  -- node[lbl,above]{126 traces} (t0);
\draw[ar] (t0)   -- (desc);
\draw[ar] (desc) -- node[lbl,above]{keep if best in cell} (arch);
\draw[ar] (arch.south) |- node[lbl,above right,pos=0.62]{front, $\le6$} (t1.west);
\draw[densely dotted,gray!70] (db.west)  -- (arch.north east);
\draw[densely dotted,gray!70] (db.east)  -- (desc.north west);
\draw[densely dotted,gray!70] (db.south) -- (t1.north);
\end{tikzpicture}
\caption{One generation of the loop. A proposer draws a parent from the
archive's occupied cells and emits a variant; Tier~0 scores it on the 126
search traces in the CBP-NG simulator, and the variant is kept only if it is
the best design in its cell of the $(\epi{},p_1,p_2)$ grid, where \epi{} is
energy per instruction and $p_1,p_2$ are the two prediction levels' latencies.
Insertion happens first: Tier~1 then re-builds at most six designs from the
archive's Pareto front and checks that they survive translation to ChampSim, so
it gates portability and never the score. Each node is a separate container
image and machine pool, and the lineage database is the only state they share.}
\label{fig:loop}
\end{figure}
...
window. All 41 designs the rule-based arms promoted passed, although the port
departs structurally
...
behaviorally distinct designs rather than a single incumbent. Each rule-based
proposal perturbs two of eight TAGE template parameters within declared ranges,
so the proposer can resize the predictor but never change its algorithm. The
rule-based arms ran 40
...
Generations 0--10 are a shared trunk, and it and the \emph{parameter arm} that
resumes it at generation~11 are the two \emph{rule-based} arms: their proposer
resizes the template inside its declared box. The \emph{design arm}, which also
forks at generation~11, instead has an agent write the predictor's source
outright. The two kinds of arm are not otherwise matched. From the fork onward
the design agent's prompt carried a digest of the CBP-NG~2025 winning entries,
naming ahead-pipelining and giving the index-early/select-late recipe, and the
agent held a CBP-NG checkout and a compiler; the rule-based proposer was given
none of this, because it could have done nothing with it. Representation and
knowledge changed together, so the design arm's scores are reported here as
a second population for the depth audit, not as evidence that a search
rediscovered anything.
...
\label{eq:cpi}
\end{equation}
where $\text{mpi}$ is mispredictions per instruction and $D=9$ is the pipeline
depth. Letting $w=TC$ denote the wrong-path
...
\label{eq:vfs}
\end{equation}
where $E=(\rho\,\epi{}/\epi{}_\text{ref}+\lambda S^{\gamma})(1+w/2)$,
$\rho=0.05$ is the predictor's share of core energy, $\lambda=1-\rho$ is the
rest of the core's, and $w_\text{ref}$ is $w$ at the reference
point~\cite{vfsmetric}.
...
designs (marks). Dotted lines bracket the $116$ configurations the rule-based
proposer reached; the parameter arm's seven final elites are shown with the
seed. The design arm's degenerate elite
($T=0.33$) is off scale.}
\label{fig:tcurve}
\end{figure}
...
$T=8.05$; at the reference point itself the throughput elasticity is already
only $+0.008$ and accuracy ($-0.035$) is the largest term, so near the reference
the score is intentionally insensitive to additional width.
...
$-0.048$ to $-0.083$, and MORSL and Fan sit almost exactly at the throughput that
...
explores, not \vfs{} itself, and a search that generalizes it into a rule will
keep buying width after the elasticity has turned against it.
...
against $5.26$--$5.63$); removing it restores $\tau=+1$ at every depth. By
...
The design arm is the strongest case: the eighteen elites of its generation-34
archive --- eleven of its own plus the seven it inherited from the trunk ---
span \mpki{} $5.26$--$11.27$ and rank least stably ($\tau=+0.58$ at $D=3$, $+0.77$ at
...
($\tau=-0.370$), in the 126- and 168-trace subsets alike. The inversion itself
...
removes it ($\tau=+0.290$ at $D=3$), leaving the archives' ordering unchanged. Replaying selection itself at other depths, with the
...
several depths, which Eq.~\eqref{eq:cpi} makes free.
...
(MORSL's $C$ is inferred); this paper measures a loop, not an entry. The design
arm is neither matched nor paired to the parameter arms: it ran under a single
seed and not concurrently, Tier~1 was run on the rule-based arms alone, and its
held-out fraction was zero from generation~15, so
