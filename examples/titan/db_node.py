"""The `database` ray node: owns the durable Titan log store.
...
Layout produced under TITAN_DB_ROOT:
    ...
    └── <ext>-<run_tag>/           # one per pipeline (archived work_root)
        ├── implementations/       #   per-iteration Saturn diffs
        ├── llm_logs/              #   claude session transcripts
        ├── sim_logs/              #   per-iteration DUT test logs
        ├── checklist.md  knowledge.md
        └── ...

Titan runs one extension (`ext` == "ime"), so `<ext>` is always `ime/` here —
the layout stays keyed by extension, as in riscv_extensions, so this store can
hold more than one run label without a schema change.
"""
...
from constants import DB_ROOT_DEFAULT, DB_ROOT_ENV

# Overridable so a deployment can point elsewhere; the `database` node must have
# local write access here.
DB_ROOT = os.environ.get(DB_ROOT_ENV, DB_ROOT_DEFAULT)
...
# TODO(titan): ELFs under DB_ROOT/tests/<set>/ are staged by hand from the
# chipyard image's prebuilt riscv-tests / riscv-vector-tests. The principled
# producer is a test-suite-builder node compiling from source for any
# extension; fetch_tests stays the read path either way.
...
# ---------------------------------------------------------------------------
# Random-test pool (S3): riscv_extensions fills this from a riscv-dv generator
# node, but riscv-dv cannot reach vector instructions at all, so Titan has no
# generator node — ime_stress.py enumerates legal tile geometries and emits
# self-checking paired assembly programs on the head, then registers them here
# exactly as a generator would. The stress_test streams pooled tests to cosims
# and marks pending -> passed. A divergence resets every status so the whole
# batch (failing test included) re-runs after the fix. The pool is SCRATCH,
# living under DB_ROOT/tmp/<run_id>; pool_finalize folds the .S record into
# the sweep and deletes it before the run's logging ends.
# ---------------------------------------------------------------------------
